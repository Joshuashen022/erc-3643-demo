...
library OwnershipTransferLib {
    ...
    function transferAllOwnerships(
        ...
        _transferOwnership(
            vm, address(identityDeployment.claimIssuerGateway), owners.claimIssuerGateway, "claimIssuerGateway"
        );
        _transferOwnership(
            vm, address(identityDeployment.claimIssuerIdFactory), owners.claimIssuerIdFactory, "claimIssuerIdFactory"
        );
        _transferOwnership(
            vm, address(identityDeployment.identityIdFactory), owners.identityIdFactory, "identityIdFactory"
        );
        ...
        _transferOwnership(
            vm, address(trexImplementationAuthority), owners.trexImplementationAuthority, "trexImplementationAuthority"
        );
        ...
        _transferOwnership(
            vm, address(suiteResult.trustedIssuersRegistry), owners.trustedIssuersRegistry, "trustedIssuersRegistry"
        );
        _transferOwnership(
            vm, address(suiteResult.claimTopicsRegistry), owners.claimTopicsRegistry, "claimTopicsRegistry"
        );
    }
    ...
    function _transferOwnership(Vm vm, address contractAddress, address newOwner, string memory contractName) private {
        ...
        if (verifiedOwner == newOwner) {
            ...
            console2.log(
                " %s ownership transfer failed: current owner is %s, expected %s", contractName, verifiedOwner, newOwner
            );
        }
    }
}
