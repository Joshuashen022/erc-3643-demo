...
contract RWAClaimIssuerTest is Test {
    ...
    function testRevokeClaim_Success() public {
        ...
        bytes32 claimId = identity.addClaim(
            CLAIM_TOPIC_KYC, CLAIM_SCHEME_ECDSA, address(claimIssuer), sig, data, "https://example.com/claim"
        );

        // Verify claim is not revoked
        (uint256 returnedTopic,,, bytes memory returnedSig,,) = identity.getClaim(claimId);
